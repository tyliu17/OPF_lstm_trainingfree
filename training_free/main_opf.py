...
output_plot(train_loss, val_loss, lr_list, output_dir)

# 輸出模型
path_model = 'C:\\Users\\USER\\Desktop\\GNN_OPF_electricity_market-main\\GNN_OPF_electricity_market-main\\GNN_OPF_electricity_market-main\\data_gen\\118_data\\FCNN_model\\'
timestamp = datetime.now().strftime('%m%d%H%M')
path_model = path_model+'nn_%s.pt'%(timestamp)

if feas==False: path_model.replace('feas','')
torch.save(net.state_dict(),path_model)

print(net)
# from evaluate_model import evaluate_model
# # 進行模型評估
# y_pred1, y_test2 = evaluate_model(net, x_test, y_test, n_bus, vy_scale, vy_deviation, device)
